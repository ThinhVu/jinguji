<template>
  <div>
    <div class="fr ai-c fg-4px fs-s h-30px c:#1F2328">
      ...
      <t-btn v-if="hasProps"
             @click="toggleExpand"
             primary style="border-radius: 100px; width: 20px; height: 20px; padding: 0px">
        <t-icon v-if="collapsed">fas fa-chevron-down@16px:#fff</t-icon>
        <t-icon v-else>fas fa-chevron-up@16px:#fff</t-icon>
      </t-btn>
      <t-btn v-if="!hasProps || collapsed"
             @click="addKV"
             primary class="fr ai-c jc-c"
          style="border-radius: 100px; width: 20px; height: 20px; padding: 0px">
        <t-icon>fas fa-plus@16px:#fff</t-icon>
      </t-btn>
    </div>
    <div v-if="!collapsed && hasProps" class="grid gtc-1fr-1fr-50px mt-1" style="grid-column-gap: 4px; grid-row-gap: 12px">
